# Third-Party Notices

PPSSPP Switch Community Build v0.7.1 is distributed under the GNU General
Public License, version 2.0 or later. The complete license text is included as
`LICENSE.TXT` in the binary package.

## NXVK and Mesa

This release statically links NXVK commit
`69ec283dbda64e65347a36274efb349122e85363`, based on Mesa 26.1.4. NXVK's new
files are GPL-2.0-or-later and upstream Mesa files retain their respective
licenses. The applicable NXVK and Mesa license texts are included in the binary
package under `licenses/nxvk/`.

GitHub provides source snapshots for each release tag. The exact source
revisions used by a build are recorded in `BUILD-METADATA.txt`.

## FFmpeg

This release uses the PPSSPP FFmpeg submodule at commit
`82049cca2e4c1516ed00a77b502a21f91b7843f4`.
